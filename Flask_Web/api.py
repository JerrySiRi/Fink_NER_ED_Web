...
def query():
    ...
    # BUG 现在是undefined的文本
    # 从 HTTP 请求中获取名为 text 的值，这个值应该包含要处理的文本数据。【从网页中获取文本】
    if not text:
        # 检查获取的文本数据是否为空，如果为空，将 result 键设置为 "error"
    ...
    path_test = '../test.json'
    with open(path_test, 'w', encoding='utf-8') as _:  # 清空文件内容，准备写入新的测试数据
        pass
    ...
    recognized_entity = test()#【【需要调用已有模型，返回的可能是字符串 or pickle文件的列表】】
    res = recognized_entity
